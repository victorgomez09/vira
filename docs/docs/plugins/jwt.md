...
@jwt_requires_role(role)Ensures the user has a valid JWT and that the token's payload contains the specified role in its list of roles.Failure ConditionHTTP StatusMissing/Invalid token401 UnauthorizedToken valid, but role missing403 ForbiddenPythonfrom plugins.jwt.decorators import jwt_requires_role
